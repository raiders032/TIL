...
* suspended(stopped): 외부적인 이유로 프로세스의 수행이 정지된 상태
  ...
  * 예) 프로세스가 중기 스케줄러에 의해 메모리에서 디스크로 swap out된 경우
...
* 메모리의 여유 공간을 마련하기 위해 어떤 프로세스를 메모리에서 디스크로 쫓아낼지를 결정한다.
...
![image-20210415205321469](./images/image-20210415205321469.png)
