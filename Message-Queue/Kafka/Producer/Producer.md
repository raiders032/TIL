...
* 프로듀서가 카프카로 전송한 메시지는 해당 토픽 내 각 파티션의 로그 세그먼트에 저장된다.
  * 로그 세그먼트 참고 [Kafka.md](../Kafka/Kafka.md)
...
- 기본값 `false`
- 이 옵션을 `true` 로 설정하면 아래의 옵션도 반드시 변경해야 한다. 아니면 오류 발생

`max.in.flight.requests.per.connection= 1~5`

- ACK를 받지 않은 상태에서 하나의 커넥션에서 보낼 수 있는 최대 요청 수
- 기본값 5이며 5 이하로 설정 한다.
...
- ACK를 받지 못한 경우 재시도를 해야 하므로 0보다 큰 값을 설정한다.
