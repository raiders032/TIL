...
* 웹 브라우저는 인터넷 프로토콜(IP) 주소를 통해 상호작용한다.
  ...
  * 사람은 naver.com과 같은 도메인 이름을 통해 온라인으로 정보에 액세스한다. 
...
* 인터넷이 상용화되어 더이상 hosts 파일로 관리할 수 없게되자 DNS가 생겨났다.
  * hosts 파일은 로컬에서 관리된다. 
  * 따라서 도메인에 해당하는 IP 주소를 변경하려면 모든 호스트의 hosts 파일을 업데이트 해야하지만 이는 불가능하다. 그래서 도메인과 IP 주소를 관리하는 중앙 집중 저장소가 필요했는데 이것이 DNS이다.
