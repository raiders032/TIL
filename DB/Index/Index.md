...
* 인덱스는 조회 성능을 향상 시킬 수 있는 자료구조
  ...
  * 또한 인덱스 자료구조를 관리하기 위한 추가적인 저장 공간을 소비 -> 인덱스 파일
* 인덱스는 하나 이상의 컬럼을 가지고 생성할 수 있다
* 인덱스에는 크게 두가지 종류가 있다.
...
* 탐색키를 기준으로 데이터 파일이 정렬되어 있으면 Primary Index 그렇지 않으면 Secondary Index라고 한다



## 2.1 기본 인덱스(Primary Index)
...
## 2.3 단일 단계 인덱스(Single level Index)

* 단일 단계 인덱스란 순차 파일을 인덱싱해서 인덱스를 통해 임의의 레코드를 접근할 수 있는 파일



## 2.4 다단계 인덱스(Mutilevel Index)
