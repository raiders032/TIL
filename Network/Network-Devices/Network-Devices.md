...
* 기본적으로 1:1 변환이 기본이지만 IP 주소 고갈로 여러개의 IP 주소를 하나의 IP 주소 변환하기도 한다
  * 아래응 보다 정확한 용어
  * 1:1 변환 NAT
  * N:1 변환 PAT(Port Address Translation)
* NAT는 사설 IP 주소에서 공인 IP 주소로 전환할 때 가장 많이 사용된다
  * [IP.md](../Protocol/IP/IP.md) (사설 IP 주소,  공인 IP 주소)참조
  * IPv4 주소의 고갈의 문제를 NAT를 사용해 해결했다



**NAT 동작과정(공인 IP 주소와 사설 IP 주소)**

* 출발지 사용자(10.10.10.10 사설 IP 주소)가 목적지 웹서버(20.20.20.20 공인 IP 주소)와 통신하는 과정
* 클라이언트의 포트는 2000 웹서버의 포트는 80

1. 사용자가 출발지를 `10.10.10.10` 목적지를 `20.20.20.20` 로 설정한 패킷을 전송한다
2. NAT 역할을 수행하는 장비에서 사용자가 보낸 패킷을 받고 외부 네트워크와 통신하기 위해 공인 IP인 `11.11.11.11` 로 출발지 IP 주소를 변경한다(변경전 IP 주소와 변경 후 IP 주소를 NAT 테이블에 기록한다)
3. NAT는 출발지가 `11.11.11.11` 인 패킷을 전송한다
4. 패킷을 수신한 서버가 출발지는 `20.20.20.20` 목적지는 `11.11.11.11`인 패킷을 응답으로 보낸다
5. 웹 서버의 응답을 받은 NAT는 NAT 테이블에서 목적지 테이블과 `11.11.11.11` 과 매핑되는 원래 출발지  `10.10.10.10` 주소를 찾는다
6. NAT는 패킷의 목적지를 `11.11.11.11` 에서 `10.10.10.10` 로 변경해 최종적으로 사용자에 패킷을 전달한다
