...
- 따라서 ID의 중복을 막기 위한 별도의 중앙 기관이 존재하지 않아도 된다
	- Sequential ID는 distributed systems에서 유일성을 만족시키기 위해 별도의 중앙 기관이 필요하다. 
...
- 버전 1과 유사하지만, POSIX UID와 GID를 추가로 포함할 수 있다.
- DCE (Distributed Computing Environment)의 보안 요구 사항을 충족하기 위해 설계되었다.
...
![[Pasted image 20230814132639.png]]

- 이전 버전의 UUID는 시간 순서로 정렬되지 않는 성질이 있다. 
- 이는 UUID로 인덱싱을 설정했을 때 성능에 문제가 된다.
- UUID V6는 UUID V1의 구조를 재배치해서 시간 순서로 정렬이 가능한 UUID이다.
- UUID1에서 사용되는 **그레고리력 에포크 타임스탬프**를 사용하기 때문에 UUID V1과 호환된다.
	- 총 60비트로 시간을 표현한다.
	- 1582년 10월 15일 00:00:00.00 부터 시작한다.
	- UUID 버전 1과 6에서는 100-나노초의 정밀도를 가지고 있다.

<br>

## 5.6 UUID V7

![[Pasted image 20230920200329.png]]

- UUID V7는 새로 개발된 시스템에서 사용하기 적합하다.
	- 즉 향후 호환성이 필요하지 않은 경우에 적합합니다.
- UUID1이 그레고리력(Gregorian) 에포크 타임스탬프를 사용하는 반면, UUID7은 유닉스(Unix) 에포크 타임스탬프를 사용하도록 정의되어 있기 때문에 하위 버전과 호환되지 않는다.
- 시간을 48비트로 표현하며 밀리초 정밀도를 가지고 있다.


> [!NOTE] 유닉스 에포크 타임스탬프
> 유닉스 타임스탬프는 1970년 1월 1일 0시 0분 0초 UTC를 시작점으로 한다. 이 때를 유닉스 에포크라고 부르며, 이후의 시간은 이 시점부터의 초 단위로 표현된다. 예를 들어, 유닉스 타임스탬프 값이 1이면 유닉스 에포크 이후 1초가 지난 시간을 의미한다.

> [!NOTE] 그레고리력 에포크 타임스탬프
> '그레고리력 에포크'라는 공식적인 표준은 없다. 그레고리력은 현재 국제적으로 널리 사용되는 태양력 달력 시스템이다. 그레고리력 자체에는 디폴트 '에포크'가 없다. 따라서 어떤 컴퓨터 시스템이나 애플리케이션에서 그레고리력을 기반으로 한 타임스탬프를 사용할 때, 그들만의 시작점을 정할 수 있다.

<br>

## 5.7 UUID V8

![[Pasted image 20230920201908.png]]
